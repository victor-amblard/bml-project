...
def eval(loader, model, criterion, cuda=False, regression=False, verbose=False, eval=True):
    loss_sum = 0.0
    correct = 0.0
    stats_sum = defaultdict(float)
...
def predict(loader, model, cuda = False, verbose=False):
    predictions = list()
    targets = list()
...
def predictions(test_loader, model, seed=None, cuda=False, regression=True, **kwargs):
